...
import static com.rb.nonbiz.math.vectorspaces.RBMatrixUtils.isSimilartoIdentityMatrix;
...
public class RBMatrixUtilsTest
  ...
  @Test
  public void testIsOrthoNormalTransformationMatrixWithCovaiance() {
    // Covariance matrix with reasonably strong positive correlation
    DoubleMatrix2D covMat = matrix2by2(1.0, 0.5, 0.5, 1.0);
    double epsilon = 1e-4;
    ...
    DoubleMatrix2D orthToRaw = matrix2by2(0.57737, -1, 0.57737,1);
    ...
    //   variance( [1, 1]) = 2.0
    assertEquals(computeVariance(covMat, new Algebra().mult(orthToRaw, matrix2by1(1,0))), 1.0, 0.0001);
    assertEquals(computeVariance(covMat, new Algebra().mult(orthToRaw, matrix2by1(0,1))), 1.0, 0.0001);
    assertEquals(computeVariance(covMat, new Algebra().mult(orthToRaw, matrix2by1(1,1))), 2.0, 0.0001);
  }
  @Test
  public void testIsOrthoNormalTransformationMatrixNoCovaiance() {
    ...
    assertTrue(isOrthoNormalTransformationMatrix(singletonMatrix(5.0), singletonMatrix(1), 1e2));
  ...
  public void testIsSimilartoIdentityMatrix() {
    ...
    assertTrue(isSimilartoIdentityMatrix(rbIdentityMatrix(1), smallEpsilon));
    assertTrue(isSimilartoIdentityMatrix(rbIdentityMatrix(2), smallEpsilon));
    assertTrue(isSimilartoIdentityMatrix(rbIdentityMatrix(4), smallEpsilon));
    assertTrue(isSimilartoIdentityMatrix(rbIdentityMatrix(10), smallEpsilon));

    // epsilon tests
    assertTrue(isSimilartoIdentityMatrix(
        rbMatrix2by2(1.0+smallEpsilon, 0.0, 0.0, 1.0-smallEpsilon),
        largeEpsilon));
    assertTrue(isSimilartoIdentityMatrix(
        rbMatrix2by2(3.0, -4.0, 8.0, 0.0),
        10.0)); // Huge epsilon is forgiving
    assertFalse(isSimilartoIdentityMatrix(
    ...
    // Identity matrix
    assertTrue(isSimilartoIdentityMatrix(rbMatrix2by2(1.0, 0.0, 0.0, 1.0), largeEpsilon));
    // Lots of non-identity matrices
    assertFalse(isSimilartoIdentityMatrix(rbMatrix2by2(1.0, 0.0, 2*largeEpsilon, 1.0), largeEpsilon));
    assertFalse(isSimilartoIdentityMatrix(rbMatrix2by2(2.0, 0.0, 0.0, 2.0), largeEpsilon));
    assertFalse(isSimilartoIdentityMatrix(rbMatrix2by2(-1.0, 0.0, 0.0, -1.0), largeEpsilon));
    assertFalse(isSimilartoIdentityMatrix(rbMatrix2by2(-1.0, 0.0, 0.0, -2.0), largeEpsilon));
    assertFalse(isSimilartoIdentityMatrix(rbMatrix2by2(-1.0, 0.1, 0.1, 1.0), largeEpsilon));
    assertFalse(isSimilartoIdentityMatrix(rbMatrix2by2(-1.0, -0.1, -0.1, 1.0), largeEpsilon));
    assertFalse(isSimilartoIdentityMatrix(rbMatrix2by2(-1.0, 1.0, 1.0, 1.0), largeEpsilon));
  }
