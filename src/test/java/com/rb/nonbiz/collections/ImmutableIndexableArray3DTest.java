...
import static com.rb.nonbiz.collections.ImmutableIndexableArray3D.emptyImmutableIndexableArray3D;
import static com.rb.nonbiz.collections.ImmutableIndexableArray3D.immutableIndexableArray3D;
import static com.rb.nonbiz.collections.MutableIndexableArray3DTest.mutableIndexableArray3DMatcher;
 ...
 */
public class ImmutableIndexableArray3DTest extends RBTestMatcher<ImmutableIndexableArray3D<String, Boolean, Integer, Double>> {
  ...
  public void testArraySizePreconditions() {
    ...
    TriFunction<
        ...
        ImmutableIndexableArray3D<String, Boolean, Integer, Double>> maker =
        (xMapping, yMapping, zMapping) -> immutableIndexableArray3D(
            new Double[][][] {
    ...
    ImmutableIndexableArray3D<String, Boolean, Integer, Double> doesNotThrow = maker.apply(goodXMapping, goodYMapping, goodZMapping);
  ...
  public void testTransformValues() {
    assertThat(
        immutableIndexableArray3D(
            ...
            .transformValues(v -> Strings.format("_%s", v)),
        immutableIndexableArray3DMatcher(
            immutableIndexableArray3D(
                new String[][][] {
                    { { "_111.0", "_112.0" }, { "_121.0", "_122.0" } },
                    { { "_211.0", "_212.0" }, { "_221.0", "_222.0" } },
                    { { "_311.0", "_312.0" }, { "_321.0", "_322.0" } }
                },
                simpleArrayIndexMapping("a", "b", "c"),
                simpleArrayIndexMapping(false, true),
                simpleArrayIndexMapping(10, 11)),
  ...
  public void testTransformEntries() {
    assertThat(
        immutableIndexableArray3D(
            ...
            simpleArrayIndexMapping(false, true),
            simpleArrayIndexMapping(10, 11))
            .transformEntries( (x, y, z, v) -> Strings.format("%s_%s_%s_%s", x, y, z, v)),
        immutableIndexableArray3DMatcher(
            immutableIndexableArray3D(
                new String[][][] {
  ...
  public void testFilteredValuesStreamXYZ() {
    ImmutableIndexableArray3D<String, Boolean, Integer, Double> original = immutableIndexableArray3D(
        new Double[][][] {
  ...
  @Override
  public ImmutableIndexableArray3D<String, Boolean, Integer, Double> makeTrivialObject() {
    return emptyImmutableIndexableArray3D(new Double[][][] {});
  }

  @Override
  public ImmutableIndexableArray3D<String, Boolean, Integer, Double> makeNontrivialObject() {
  ...
  @Override
  public ImmutableIndexableArray3D<String, Boolean, Integer, Double> makeMatchingNontrivialObject() {
  ...
  @Override
  protected boolean willMatch(ImmutableIndexableArray3D<String, Boolean, Integer, Double> expected,
                              ImmutableIndexableArray3D<String, Boolean, Integer, Double> actual) {
    return immutableIndexableArray3DMatcher(expected, v -> doubleAlmostEqualsMatcher(v, DEFAULT_EPSILON_1e_8))
        .matches(actual);
  }

  public static <X, Y, Z, V> TypeSafeMatcher<ImmutableIndexableArray3D<X, Y, Z, V>> immutableIndexableArray3DMatcher(
      ImmutableIndexableArray3D<X, Y, Z, V> expected, MatcherGenerator<V> valueMatcherGenerator) {
    return makeMatcher(expected,
        match(v -> v.getMutableArray3D(), f -> mutableIndexableArray3DMatcher(f, valueMatcherGenerator)));
  }
