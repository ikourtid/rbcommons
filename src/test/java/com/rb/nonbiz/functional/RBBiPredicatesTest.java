...
public class RBBiPredicatesTest {
  ...
  public void testDoubleIsAlmostEqualTo() {
    assertFalse(doubleIsAlmostEqualTo(0.1).test(7.59, 7.7));
    assertTrue(doubleIsAlmostEqualTo(0.1).test(7.61, 7.7));
    assertTrue(doubleIsAlmostEqualTo(0.1).test(7.7, 7.7));
    assertTrue(doubleIsAlmostEqualTo(0.1).test(7.79, 7.7));
    assertFalse(doubleIsAlmostEqualTo(0.1).test(7.81, 7.7));
  ...
  public void testIsAlmostEqualTo() {
    assertFalse(RBBiPredicates.<SignedMoney>isAlmostEqualTo(0.1).test(signedMoney(7.59), signedMoney(7.7)));
    assertTrue(RBBiPredicates.<SignedMoney>isAlmostEqualTo(0.1).test(signedMoney(7.61), signedMoney(7.7)));
    assertTrue(RBBiPredicates.<SignedMoney>isAlmostEqualTo(0.1).test(signedMoney(7.7), signedMoney(7.7)));
    assertTrue(RBBiPredicates.<SignedMoney>isAlmostEqualTo(0.1).test(signedMoney(7.79), signedMoney(7.7)));
    assertFalse(RBBiPredicates.<SignedMoney>isAlmostEqualTo(0.1).test(signedMoney(7.81), signedMoney(7.7)));
  ...
  public void testPreciseValueExtendersComparisons() {
    ...
    assertTrue(RBBiPredicates.<PositiveQuantity>isAlmostEqualTo(0.10).test(positiveQuantity(10), positiveQuantity(10.01)));
    assertTrue(RBBiPredicates.<PositiveQuantity>isAlmostEqualTo(0.10).test(positiveQuantity(10), positiveQuantity(10.10)));
    assertFalse(RBBiPredicates.<PositiveQuantity>isAlmostEqualTo(0.10).test(positiveQuantity(10), positiveQuantity(10.20)));
    assertTrue(RBBiPredicates.<PositiveQuantity>isAlmostEqualTo(0.10).test(positiveQuantity(10), positiveQuantity(9.99)));
    assertTrue(RBBiPredicates.<PositiveQuantity>isAlmostEqualTo(0.10).test(positiveQuantity(10), positiveQuantity(9.90)));
    assertFalse(RBBiPredicates.<PositiveQuantity>isAlmostEqualTo(0.10).test(positiveQuantity(10), positiveQuantity(9.80)));
  ...
  public void testIsAlmostMultipliedBy() {
    assertFalse(RBBiPredicates.<Money>isAlmostMultipliedBy(1.2, 0.02).test(money(100), money(117)));
    assertTrue( RBBiPredicates.<Money>isAlmostMultipliedBy(1.2, 0.02).test(money(100), money(119)));
    assertTrue( RBBiPredicates.<Money>isAlmostMultipliedBy(1.2, 0.02).test(money(100), money(120)));
    assertTrue( RBBiPredicates.<Money>isAlmostMultipliedBy(1.2, 0.02).test(money(100), money(121)));
    assertFalse(RBBiPredicates.<Money>isAlmostMultipliedBy(1.2, 0.02).test(money(100), money(123)));

    // Cases where the denominator is 0 or almost 0
    // In these 4 cases, 1.2 * 0 = 0 (or thereabouts), so this evaluates to true
    assertTrue( RBBiPredicates.<Money>isAlmostMultipliedBy(1.2, 0.02).test(money(0),    money(0)));
    assertTrue( RBBiPredicates.<Money>isAlmostMultipliedBy(1.2, 0.02).test(money(1e-9), money(0)));
    assertTrue( RBBiPredicates.<Money>isAlmostMultipliedBy(1.2, 0.02).test(money(0),    money(1e-9)));
    assertTrue( RBBiPredicates.<Money>isAlmostMultipliedBy(1.2, 0.02).test(money(1e-9), money(1e-9)));

    // However, 1.2 * 0 != 789. But at least there's no divide-by-zero problem here.
    assertFalse(RBBiPredicates.<Money>isAlmostMultipliedBy(1.2, 0.02).test(money(0),    money(789)));
    assertFalse(RBBiPredicates.<Money>isAlmostMultipliedBy(1.2, 0.02).test(money(1e-9), money(789)));
  }

  @Test
  public void testIsAlmostMultipliedBy_negativeValues() {
    assertFalse(RBBiPredicates.<SignedMoney>isAlmostMultipliedBy(-1.2, 0.02).test(signedMoney(-100), signedMoney(117)));
    assertTrue( RBBiPredicates.<SignedMoney>isAlmostMultipliedBy(-1.2, 0.02).test(signedMoney(-100), signedMoney(119)));
    assertTrue( RBBiPredicates.<SignedMoney>isAlmostMultipliedBy(-1.2, 0.02).test(signedMoney(-100), signedMoney(120)));
    assertTrue( RBBiPredicates.<SignedMoney>isAlmostMultipliedBy(-1.2, 0.02).test(signedMoney(-100), signedMoney(121)));
    assertFalse(RBBiPredicates.<SignedMoney>isAlmostMultipliedBy(-1.2, 0.02).test(signedMoney(-100), signedMoney(123)));
