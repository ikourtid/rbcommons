 ...
 *
 * But what if you want to iterate over that iterable using more than one ordering? For example, if we iterate
 * over a collection of TaxLot objects of different instruments, maybe we want to iterate over tax lots (of
 * different instruments) in instrument ID order, and after that iterate in date order.
 *
 * This interface addresses such scenarios. The alternative would be for the class that holds a {@code Collection<TaxLot>}
 * (in this particular example) to expose such a collection, and let the caller sort and iterate over it.
 * The RBSortableIterable is a bit cleaner, because it keeps us from having to expose that collection.
 *
 * Note that T does not necessarily implement Comparable. Comparable is useful for situations where there is a single,
