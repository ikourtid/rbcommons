  ...
  private IidMap<V> copyWithInstrumentIdsRemovedHelper(IidSet toRemove, boolean throwOnMissingKeys) {
    ...
    if (throwOnMissingKeys) {
      RBSimilarityPreconditions.checkBothSame(
          ...
          // We could cast the following (a long) to an int, but would incur the cost of checking for overflow.
          totalRemoved.get(),
          // Instead, we cast the following to long, which doesn't require any overflow checks.
          (long) toRemove.size(),
