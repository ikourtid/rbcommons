...
public class InstrumentTypeMap<T> {
  ...
  private InstrumentTypeMap(T valueForEtfs, T valueForStocks, T valueForMutualFunds, Visitor<T> visitor) {
  ...
  public Stream<T> stream() {
    return Stream.of(valueForEtfs, valueForStocks, valueForMutualFunds);
  }
  ...
  public InstrumentType getInstrumentTypeWhenUnique(Predicate<T> predicate) {
    ...
    RBPreconditions.checkArgument(
        Stream.of(inEtfs, inStocks, inMutualFunds)
        .filter(v -> v)
        .count() == 1,
        "Predicate passed in must be true for exactly one category of {ETFs, stocks, mutual funds}; %s",
        this.toString());
    // This style of relying on a default is not great, but here we already checked that one of 3 booleans is true,
    ...
    return inEtfs ? etfInstrumentType() :
        inStocks  ? stockInstrumentType() :
                    mutualFundInstrumentType();
  }

  @Override
  public String toString() {
    return Strings.format("[ITM etf= %s ; stock= %s ; mutual fund= %s ITM]",
        valueForEtfs, valueForStocks, valueForMutualFunds);
  }
  ...
  public static class InstrumentTypeMapBuilder<T> implements RBBuilder<InstrumentTypeMap<T>> {
    ...
    public InstrumentTypeMap<T> buildWithoutPreconditions() {
      return new InstrumentTypeMap<>(valueForEtfs, valueForStocks, valueForMutualFunds, new Visitor<T>() {
        @Override
